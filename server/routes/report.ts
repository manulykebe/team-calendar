	...
	try {
		...
		if (user.settings?.availability) {
			...
			// If availability is an object, convert it to array format
				...
				availabilityArray = [
					{
						weeklySchedule:
							user.settings.availability.weeklySchedule,
						alternateWeekSchedule:
							user.settings.availability.alternateWeekSchedule,
		...
		if (!availabilityArray.length) {
			...
			siteData.app.defaultWeeklySchedule.forEach((daySchedule: any) => {
				...
				defaultSchedule[day] = slots.reduce(
					(acc: any, slot: string) => {
						acc[slot] = true;
						return acc;
					},
		...
		const availability = days.reduce((acc, date) => {
			...
			if (
				...
				setting.alternateWeekSchedule
			) {
				const weekNumber = Math.ceil(
					(date.getTime() - parseISO(setting.startDate).getTime()) /
						(7 * 24 * 60 * 60 * 1000)
				);
				const schedule =
					weekNumber % 2 === 0
