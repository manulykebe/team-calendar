...
import { format, addMonths, startOfMonth, isSameDay, addYears } from "date-fns";
...
export function MonthPicker({ currentMonth, onDateSelect, weekStartsOn }: MonthPickerProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [baseMonth, setBaseMonth] = useState(startOfMonth(currentMonth));
  const [isHovered, setIsHovered] = useState(false);
  
  useEffect(() => {
    if (Math.abs(baseMonth.getTime() - currentMonth.getTime()) > 1000 * 60 * 60 * 24 * 90) {
  ...
  const handlePrevYear = () => {
    setBaseMonth(prevMonth => addYears(prevMonth, -1));
  };
  ...
  const handlePrevMonth = () => {
    setBaseMonth(prevMonth => addMonths(prevMonth, -1));
  };
  ...
  return (
    <div className="relative">
      ...
      >
        <Calendar className={`w-5 h-5 transition-colors duration-200 ${isHovered ? 'text-blue-600' : 'text-zinc-600'}`} />
        
        {/* Tooltip */}
        <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs font-medium text-white bg-zinc-800 rounded whitespace-nowrap transition-opacity duration-200
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-sm font-medium">
                {format(baseMonth, 'MMM yyyy')} - {format(addMonths(baseMonth, 2), 'MMM yyyy')}
              </span>
              <div className="flex items-center space-x-1">
                <button
