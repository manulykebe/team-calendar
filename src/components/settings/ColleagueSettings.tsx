...
export function ColleagueSettings({ onClose }: ColleagueSettingsProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setCurrentUser(
          users.find(
            (u: User) => u.email === localStorage.getItem("userEmail"),
          ),
        );
      } catch (err) {
  ...
  const handleAbbrevChange = async (colleagueId: string, initials: string) => {
    if (!currentUser || !token) return;
                  ...
                  <div className="flex items-center space-x-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleAbbrevChange(colleague.id, e.target.value)
                        }
                      ...
                      <div className="mt-1 flex items-center space-x-2">
                        {DEFAULT_COLORS.map((color) => (
                          <button
                            key={color}
                            onClick={() =>
                              handleColorChange(colleague.id, color)
                            }
