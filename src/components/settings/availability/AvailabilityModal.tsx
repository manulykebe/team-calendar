...
export function AvailabilityModal({ colleague, onClose }: AvailabilityModalProps) {
  ...
  const handleViewReport = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const data = await getAvailabilityReport(token, colleague.site, colleague.id, reportYear);
      setReportData(data);
      setShowReport(true);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to fetch availability report");
    } finally {
      setLoading(false);
    }
  };
