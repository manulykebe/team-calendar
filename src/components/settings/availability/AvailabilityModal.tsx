...
import { updateUser, updateAvailabilityException } from "../../../lib/api/users";
...
export function AvailabilityModal({
  ...
  const [startDate, setStartDate] = useState(
    colleague.settings?.availability?.[0]?.startDate ||
      format(new Date(), "yyyy-MM-dd")
  );
  const [endDate, setEndDate] = useState(
    colleague.settings?.availability?.[0]?.endDate || ""
  );
  const [repeatPattern, setRepeatPattern] = useState<RepeatPattern>(
    (colleague.settings?.availability?.[0]?.repeatPattern as RepeatPattern) ||
      "all"
  );
  const [schedule, setSchedule] = useState<WeeklySchedule>(() => {
    const defaultSchedule =
      colleague.settings?.availability?.[0]?.weeklySchedule || {};
    return DAYS.reduce(
      (acc, day) => ({
        ...acc,
  ...
  const [alternateSchedule, setAlternateSchedule] = useState<WeeklySchedule>(
    () => {
      const defaultSchedule =
        colleague.settings?.availability?.[0]?.alternateWeekSchedule || {};
      return DAYS.reduce(
        (acc, day) => ({
          ...acc,
          [day]: defaultSchedule[day] || { am: true, pm: true },
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update the endDate of the last entry
      if (currentAvailability.length > 0) {
        const lastEntry = currentAvailability[currentAvailability.length - 1];
        const newEndDate = new Date(startDate);
        newEndDate.setDate(newEndDate.getDate() - 1);
        lastEntry.endDate = format(newEndDate, "yyyy-MM-dd");
      }

      const updatedAvailability = [...currentAvailability, newEntry];
