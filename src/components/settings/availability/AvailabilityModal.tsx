...
export function AvailabilityModal({
	...
	const [startDate, setStartDate] = useState(
		colleague.settings?.availability?.startDate ||
			format(new Date(), "yyyy-MM-dd")
	);
	const [endDate, setEndDate] = useState(
		colleague.settings?.availability?.endDate || ""
	);
	const [repeatPattern, setRepeatPattern] = useState<RepeatPattern>(
		(colleague.settings?.availability?.repeatPattern as RepeatPattern) ||
			"all"
	);
	const [schedule, setSchedule] = useState<WeeklySchedule>(() => {
		const defaultSchedule =
			colleague.settings?.availability?.weeklySchedule || {};
		return DAYS.reduce(
			(acc, day) => ({
				...acc,
	...
	const [alternateSchedule, setAlternateSchedule] = useState<WeeklySchedule>(
		() => {
			const defaultSchedule =
				colleague.settings?.availability?.alternateWeekSchedule || {};
			return DAYS.reduce(
				(acc, day) => ({
					...acc,
					[day]: defaultSchedule[day] || { am: true, pm: true },
